...
export function DocumentsPage() {
  ...
  const [pickStep, setPickStep] = useState(0); // 0: idle, 1: picking end, 2: complete
  ...
  const toggleCalPicker = (e: React.MouseEvent) => {
    e.stopPropagation();
    setCalOpen(prev => !prev);
    if (!calOpen) setHoverDate(null);
  };
  ...
  function getDayClass(ds: string) {
    ...
    if (ds === todayStr && !cls.includes('cal-day-start') && !cls.includes('cal-day-end')) {
      cls += ' cal-day-today';
    }
    return cls;
  }
  ...
  return (
    <div className="documents-page">
      ...
      <div className="page-header">
        <span className="page-header-title">문서 보관함</span>
      </div>
      ...
      <div className="control-bar">
        ...
        <div className="search-wrap">
          <div className="search-field-wrap">
            <select
              ...
              value={searchField}
              onChange={(e) => { setSearchField(e.target.value as 'name' | 'author'); resetPage(); }}
            >
        ...
        <div className="date-picker-wrap" ref={datePickerRef}>
          <span
            className={`date-display-field${pickStart ? ' has-value' : ''}${pickStep === 1 ? ' picking' : ''}`}
            onClick={toggleCalPicker}
          >
          ...
          <span
            className={`date-display-field${pickEnd ? ' has-value' : ''}`}
            onClick={toggleCalPicker}
          >
            <svg className="cal-field-icon" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.7"><rect x="1.5" y="2.5" width="13" height="12" rx="2" /><path d="M5 1.5v2M11 1.5v2M1.5 6.5h13" /></svg>
          ...
          <button
            ...
            type="button"
            onClick={clearDates}
            style={{ visibility: pickStart ? 'visible' : 'hidden' }}
          >
              ...
              <div className="cal-popup-grid">
                ...
                {Array.from({ length: daysInMonth }, (_, i) => {
                  ...
                  return (
                    <div
                      key={d}
                      className={getDayClass(ds)}
                      onClick={(e) => { e.stopPropagation(); onCalDayClick(ds); }}
        ...
        <button className="btn-read-action" onClick={markRead} disabled={selectedIds.size === 0} style={{ marginLeft: 'auto' }}>읽음</button>
        <button className="btn-read-action" onClick={markAllRead}>전체 읽음</button>
      </div>
            ...
            <tbody>
              {pageDocs.map(doc => {
                ...
                return (
                  <tr
                    key={doc.id}
                    className={readIds.has(doc.id) ? 'doc-read' : ''}
