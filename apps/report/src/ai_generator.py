...
def generate_weekly_report(canonical: dict, *, doc_meta: dict | None = None) -> str:
    ...
    if flow_logs_rollup or flow_logs_rejected:
        ...
        if flow_logs_port_dist:
            ...
            top_port_keys = {p.get("port") or p.get("dstport") for p in top_ports}
            rejected_ports = [p for p in flow_logs_port_dist[20:] if p.get("rejected", 0) > 0 and (p.get("port") or p.get("dstport")) not in top_port_keys]
...
⑦ 네트워크 트래픽 분석 (flow_logs 데이터 있을 때)
  ...
  - sub-heading "포트별 트래픽 분포": tbl — 포트(서비스명 병기: 443/HTTPS 등), 허용, 거부, 전송량. 비표준 포트 r-mid. 거부 건수가 1 이상인 포트는 반드시 포함. 전체 포트 중 일부만 표시하는 경우 테이블 아래에 "(전체 N개 포트 중 상위 M개)" 주석 추가
