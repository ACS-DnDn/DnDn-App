...
① Overview
  ...
  - tbl-summary (KPI 카드): 총 비용(cost_explorer_summary.total_cost_usd), 운영 경보(cloudwatch_rollup: ALARM/전체), 보안 Findings(access_analyzer_rollup.total_findings), 리소스 점검(advisor_rollup.total_checks), 네트워크 거부 비율(flow_logs_rollup.reject_ratio_pct — 1% 미만이면 st-ok, 1~5% st-warn, 5% 이상 st-crit)
...
⑦ 네트워크 트래픽 분석 (flow_logs 데이터 있을 때)
  ...
  - sub-heading "트래픽 개요": tbl-info 2×2 그리드로 총 허용 트래픽(건), 총 거부 트래픽(건), 거부 비율(%), 모니터링 VPC 수, 총 전송량(바이트를 읽기 쉬운 단위 KB/MB/GB로 변환)
    - 거부 비율 0% → st-ok "정상", 0~1% → st-ok, 1~5% → st-warn "주의", 5% 이상 → st-crit "경고"

  - sub-heading "일별 트래픽 추이": tbl로 날짜(KST), 총 건수, 허용, 거부, 전송량
    - 날짜별 추이를 통해 트래픽 급증/급감 여부를 보여줌
    - 특정 일에 평균 대비 2배 이상 변동이 있으면 해당 행에 주석 (예: "평소 대비 급증")

  - sub-heading "Top Talkers — 트래픽 최다 IP": tbl로 IP 주소, 요청 건수, 전송량(읽기 쉬운 단위)
    - 어떤 내부 리소스가 네트워크를 가장 많이 사용하는지 보여줌
    - 가능하면 IP 대역으로 서브넷/용도 추정 코멘트 (예: 10.250.1.x → Private 서브넷)

  - sub-heading "외부 통신 상위 목록": tbl로 외부 IP, 요청 건수, 전송량
    - RFC1918(사설 IP) 이외의 외부 목적지와의 통신을 보여줌
    - 알려진 AWS 서비스 IP 대역(예: S3, CloudFront)이면 서비스명 추정 표시
    - 알 수 없는 외부 IP가 대량 통신하면 r-mid 또는 r-hi로 주의 표시

  - sub-heading "포트별 트래픽 분포": tbl로 포트(잘 알려진 포트는 서비스명 병기: 443/HTTPS, 22/SSH, 80/HTTP 등), 허용 건수, 거부 건수, 총 전송량
    - 비표준 포트에 대량 트래픽이 있으면 r-mid로 표시

  - sub-heading "프로토콜 분포": tbl로 프로토콜명(TCP/UDP/ICMP), 건수, 전송량, 비율(%)
    - ICMP가 비정상적으로 많으면 r-mid (ping sweep/scan 가능성)

  - sub-heading "거부 트래픽 분석" (거부 트래픽이 있을 때만 표시): tbl로 소스IP, 목적지포트, 거부 횟수, 위험도(r-hi: 100회 이상/r-mid: 10회 이상/r-low: 그 외)
    - 반복적인 거부는 포트 스캔 또는 무단 접근 시도 가능성 → 해당 IP에 대한 조치 권고 코멘트
