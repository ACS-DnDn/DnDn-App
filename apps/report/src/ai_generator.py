...
def generate_weekly_report(canonical: dict, *, doc_meta: dict | None = None) -> str:
    ...
    if flow_logs_rollup or flow_logs_rejected:
        ...
        if flow_logs_port_dist:
            # 상위 20개 + 거부가 있는 포트는 반드시 포함
            _pk = lambda p: p.get("dst_port") or p.get("port") or p.get("dstport")
            _is_rej = lambda p: str(p.get("action", "")).upper() == "REJECT" or p.get("rejected", 0) > 0
            top_ports = flow_logs_port_dist[:20]
            top_port_keys = {_pk(p) for p in top_ports}
            rejected_ports = [p for p in flow_logs_port_dist[20:] if _is_rej(p) and _pk(p) not in top_port_keys]
            selected_ports = top_ports + rejected_ports
            note = f" (전체 {len(flow_logs_port_dist)}개 포트 중 {len(selected_ports)}개 표시)" if len(flow_logs_port_dist) > len(selected_ports) else ""
            ext_section += f"### 포트별 트래픽 분포{note}\n{json.dumps(selected_ports, ensure_ascii=False, indent=2)}\n"
        if flow_logs_proto:
            ext_section += f"### 프로토콜 분포\n{json.dumps(flow_logs_proto, ensure_ascii=False, indent=2)}\n"
        if flow_logs_rejected:
            ext_section += f"### 거부 트래픽 소스 ({len(flow_logs_rejected)}건)\n{json.dumps(flow_logs_rejected, ensure_ascii=False, indent=2)}\n"
    if events_summary:
...
② 운영 경보(ALARM) 현황 (cloudwatch_alarms / cloudwatch_rollup 데이터 있을 때)
  ...
  - ALARM 상태인 것을 상단에 표시
  - state_value가 "ALARM (recovered)"인 항목은 보고 기간 중 경보가 발생했으나 현재 복구된 알람. st-warn으로 표시하고 상태 셀에 "복구됨" 표기
...
⑦ 네트워크 트래픽 분석 (flow_logs 데이터 있을 때)
  ...
  - sub-heading "포트별 트래픽 분포": tbl — 포트(서비스명 병기: 443/HTTPS 등), 허용, 거부, 전송량. 비표준 포트 r-mid. 거부 건수가 1 이상인 포트는 반드시 포함. 전체 포트 중 일부만 표시하는 경우 테이블 아래에 "(전체 N개 포트 중 M개 표시)" 주석 추가

  - sub-heading "프로토콜 분포": tbl — 프로토콜(TCP/UDP/ICMP), 건수, 전송량, 비율(%). ICMP 과다 시 r-mid

  - sub-heading "거부 트래픽 분석" (거부 0건이면 생략 가능): tbl — 소스IP, 목적지포트, 횟수, 위험도(r-hi≥100/r-mid≥10/r-low). 제공된 거부 트래픽 데이터는 누락 없이 전부 표시
