...
⑦ 네트워크 트래픽 분석 (flow_logs 데이터 있을 때)
  - sub-heading "트래픽 개요": flow_logs_rollup — 총 허용/거부 수, 거부 비율(%), VPC 수
  - sub-heading "거부 트래픽 Top 소스": tbl로 소스IP, 목적지포트, 거부 횟수, 위험도(r-hi: 100회 이상/r-mid: 10회 이상/r-low: 그 외)
  - sub-heading "포트별 트래픽 분포": tbl로 포트, 허용/거부 수, 총 바이트
  - 거부 비율 1% 이상이면 st-warn, 5% 이상이면 st-crit
