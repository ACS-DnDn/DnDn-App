...
DEV_USER_ID = os.getenv("DEV_USER_ID", "user-001")
...
async def get_current_user(
    ...
    DEV_MODE=true 일 때는 토큰 검증 없이 DEV_USER_ID 유저를 반환한다.
    """
    ...
    if DEV_MODE:
        ...
        if user is None:
            raise HTTPException(status_code=401, detail="DEV_USER_ID not found in DB")
        return user
    # ──────────────────────────────────────────────────────
